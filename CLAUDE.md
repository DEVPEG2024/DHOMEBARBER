...
- **Tests dans le dépôt** : `cd dhomebarber-api && npm test` — 10 harnais (373 vérifications depuis le textile, 9 sept. 2026), sans dépendance ni accès à la base de production (le pool, `http2` et `fetch` sont remplacés avant le chargement du code testé)
...
- **Tests backend** : `test/test-textile.js` (105 vérifications : accès, projection client, vote, alerte, réservation avec faux pool transactionnel, cancel, notify, push « prête », `runTextileTick`). `npm test` → 10 harnais
