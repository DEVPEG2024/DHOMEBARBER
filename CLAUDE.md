...
| Partner | partners | name, category (food / sport / style / beauty / auto / leisure / services / other), tagline, description, logo_url, cover_image_url, address, city, phone, website, instagram, offers (JSONB [{id, title, percent 1..100 ou null, details}], ≤ 8), promo_code, conditions, valid_until (DATE), is_featured, is_active, sort_order — **backend à déployer**, voir `docs/backend-partners.md` |
...
### Bons plans du Gang (ajouté le 25 sept. 2026, frontend seulement)
...
- **Backend non déployé** au 25 sept. 2026 : le dépôt `dhomebarber-api` n'était pas accessible depuis la session cloud. Correctif complet dans `docs/backend-partners.md` (table, mapping, politique de lecture, règles admin, `validatePartner`, ajout aux colonnes du ramasse-miettes des médias). En attendant, `fetchPartners` traite le 400 comme une liste vide : la section est invisible et l'admin affiche un bandeau
