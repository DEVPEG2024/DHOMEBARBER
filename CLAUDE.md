...
- **Réservation** (`Booking.jsx`) : étapes qui glissent dans le sens de la navigation (`AnimatePresence mode="popLayout"`, variants avec `direction`), halo d'étape qui glisse (`layoutId="step-halo"`), coches à ressort et lignes qui se remplissent, prestations / dates / créneaux en cascade, **récapitulatif** sous les étapes (chips + prix qui compte, `AnimatedNumber`) avant la confirmation, **créneau choisi qui se loge dans le résumé** (`layoutId` `slot-HH:MM` partagé entre le bouton et le résumé), **écran de chargement** pendant l'envoi (`LoadingOverlay`, exporté : lame de rasoir sombre, arc de 34 graduations qui s'allument, pourcentage, barre fine, « Veuillez patienter » ; progression = motion value simulée jusqu'à 88 % en 2,4 s, affichée au moins `LOADING_MIN_MS` 1,5 s, puis 100 % à la réponse du serveur, pause 0,3 s), puis écran de succès (`SuccessOverlay`, exporté) = **scène de coupe** `HaircutScene` : profil d'homme en SVG, la tondeuse remonte de la nuque vers la tempe par petits coups pendant `CUT_DURATION` (2,6 s) et le dégradé se dessine derrière elle (rect de cheveux pleins qui rétrécit par `scaleY` origine haute, au-dessus d'un dégradé cheveux → peau, le tout découpé par `clipPath`), petits cheveux qui tombent, coche à ressort quand la coupe est finie, barre de progression ; `SUCCESS_DURATION` = 4,6 s avant la redirection vers Mes rendez-vous. Pas de confettis ni de lames (remplacés à la demande)
